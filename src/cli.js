...
if (args.length === 1 && args[0] === '--help') {
  ...
  try {
    ...
    for (let i = 2; i < args.length; i += 2) {
      if (args[i] !== '--mask-path' || !args[i + 1]) throw new Error('Use --mask-path <literal-path>');
      maskPaths.push(args[i + 1]);
    }
    const stat = await fs.stat(args[1]);
    if (stat.size > 8 * 1024 * 1024) throw new Error('Result file exceeds 8 MiB');
    console.log(renderReport(JSON.parse(await fs.readFile(args[1], 'utf8')), { maskPaths }));
  } catch (error) { console.error(`repropack: ${error.message}`); process.exitCode = 2; }
...
} else { console.error('Usage: repropack create <recipe.json> | verify <package-directory> | run <package-directory> --allow-execution | report <result.json> [--mask-path <path>] | --help'); process.exitCode = 2; }
